...
import { getAgendaBookings } from "@/app/_actions/get-agenda-bookings";
import { POLL_INTERVAL_MS } from "@/lib/realtime";
import { Button } from "@/app/_components/ui/button";
import { NewBookingSheet } from "./agenda/new-booking-sheet";

type Item = { id: string; startAt: string; customer: string; service: string; staff: string; status: string; priceInCents: number; discountInCents: number; paymentReceivedInCents: number; referencePhotoUrls: string[] };
...
function ReferencePhotoThumbs({ urls }: { urls: string[] }) {
  const t = useTranslations("dashboard.agenda");
  if (!urls.length) return null;
  return (
    <div className="mt-1.5 flex gap-1" title={t("referencePhotosTooltip")}>
      {urls.map((url) => (
        // eslint-disable-next-line @next/next/no-img-element -- miniatura de anexo do Cloudinary, sem next/image por simplicidade (mesmo padrão de outras imagens de tenant neste arquivo)
        <img key={url} src={url} alt="" className="size-8 rounded object-cover" />
      ))}
    </div>
  );
}
...
export function Agenda({ bookings: initialBookings, timezone, locale, nowISO, role, services, staff }: { bookings: Item[]; timezone: string; locale: string; nowISO: string; role: string; currency: string; services: ServiceOption[]; staff: StaffOption[] }) {
  ...
  const canCreateWalkIn = ["owner", "manager", "receptionist"].includes(role);

  // Padding de 1 dia pra fora da janela visível: agenda/page.tsx (SSR)
  // busca com o mesmo padding, então a janela do primeiro render (offset=0)
  // bate exatamente com a do polling — sem isso, um booking perto da borda
  // do fuso do tenant podia cair no dia certo em localDay() mas ficar fora
  // do range buscado.
  const fromISO = useMemo(() => new Date(days[0]!.getTime() - 86400000).toISOString(), [days]);
  const toISO = useMemo(() => new Date(days[6]!.getTime() + 86400000).toISOString(), [days]);
  const { data: bookings = [] } = useQuery({
    queryKey: ["dashboard-agenda", fromISO, toISO],
    queryFn: async () => {
      const result = await getAgendaBookings({ fromISO, toISO });
      if (!result?.data) throw new Error("failed to load agenda");
      return result.data;
    },
    initialData: offset === 0 ? initialBookings : undefined,
    refetchInterval: POLL_INTERVAL_MS.agenda,
  });
  return <section className="p-5 md:p-8"><header className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-medium text-primary">{t("operation")}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">{t("title")}</h1></div><div className="flex gap-2"><Button variant="outline" size="sm" aria-label={t("previous")} onClick={() => setOffset((value) => value - 1)}><ChevronLeft className="size-4" /></Button><Button variant="outline" size="sm" onClick={() => setOffset(0)}>{t("today")}</Button><Button variant="outline" size="sm" aria-label={t("next")} onClick={() => setOffset((value) => value + 1)}><ChevronRight className="size-4" /></Button>{canCreateWalkIn && <NewBookingSheet services={services} staff={staff} />}</div></header><div className="grid gap-3 lg:grid-cols-7">{days.map((day) => { const dayBookings = bookings.filter((item) => localDay(item.startAt, timezone) === localDay(day, timezone)); return <div className="min-h-56 rounded-xl border bg-card" key={day.toISOString()}><h2 className="border-b px-3 py-3 text-sm font-semibold capitalize">{formatter.format(day)}<span className="ml-2 text-xs font-normal text-muted-foreground">{dayBookings.length}</span></h2><div className="grid gap-2 p-2">{dayBookings.map((item) => <article className="rounded-lg bg-muted/60 p-2.5 text-xs" key={item.id}><div className="flex items-start justify-between gap-2"><b>{new Intl.DateTimeFormat(locale, { timeStyle: "short", timeZone: timezone }).format(new Date(item.startAt))}</b><span className="rounded bg-background px-1.5 py-0.5 text-[10px] text-muted-foreground">{item.status}</span></div><p className="mt-1 font-medium">{item.service}</p><p className="mt-0.5 text-muted-foreground">{item.customer} · {item.staff}</p><ReferencePhotoThumbs urls={item.referencePhotoUrls} /><BookingActions booking={item} role={role} /></article>)}{!dayBookings.length && <p className="px-2 py-5 text-xs text-muted-foreground">{t("noAppointments")}</p>}</div></div>; })}</div></section>;
}
