...
export async function BusinessCard({ business }: { business: Business }) {
  ...
  return (
    <ChooseBusinessLink
      ...
      className="group relative block h-52 w-64 shrink-0 overflow-hidden rounded-2xl bg-muted"
    >
      {image ? (
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-1.5 p-4">
        <span className="inline-flex w-fit items-center gap-1 rounded-full bg-primary px-2 py-0.5 text-[11px] font-semibold text-primary-foreground">
          <Icon className="size-3" />
          {t(business.organization.category)}
        </span>
        <p className="text-base font-bold text-white">{business.organization.name}</p>
