...
import type {
  ...
  InvitationNotificationJob,
  QuoteRequestNotificationJob,
} from "@/lib/notifications";
...
async function sendAndLog(params: {
  ...
  if (error) throw new Error(`Resend rejected notification: ${error.message}`);
}

const notificationsWorker = new Worker<BookingNotificationJob>(
...
const invitationsWorker = new Worker<InvitationNotificationJob>(
...
invitationsWorker.on("failed", (job, err) => {
  logger({
    jobId: job?.id,
    queue: "invitation-notifications",
    attemptNumber: job?.attemptsMade,
  }).error({ err }, "job.failed");
  Sentry.captureException(err);
});
...
const quoteRequestsWorker = new Worker<QuoteRequestNotificationJob>(
  "quote-request-notifications",
  async (job) => {
    const quoteRequest = await getQuoteRequestForNotification(
      job.data.quoteRequestId,
    );
    ...
    const vars = {
      ...
      organization: quoteRequest.organization.name,
    ...
    await sendAndLog({
      ...
      email: {
        from,
        to: ownerEmail,
        subject: t("quoteRequest.subject", vars),
        html,
        text,
      },
    });
...
quoteRequestsWorker.on("failed", (job, err) => {
  logger({
    jobId: job?.id,
    queue: "quote-request-notifications",
    attemptNumber: job?.attemptsMade,
  }).error({ err }, "job.failed");
  Sentry.captureException(err);
});
...
const mfaReminderWorker = new Worker(
  "mfa-reminder-maintenance",
  async () => {
    ...
    for (const member of members) {
      ...
      const date = new Intl.DateTimeFormat(member.user.locale ?? "pt", {
        dateStyle: "full",
        timeStyle: "short",
      }).format(deadline);
      ...
      try {
        await sendAndLog({
          ...
          email: {
            ...
            subject: t("mfaGracePeriodReminder.subject", vars),
            html,
            text,
          },
        });
        ...
        logger({ memberId: member.id }).error(
          { err },
          "mfa_reminder.send_failed",
        );
...
mfaReminderWorker.on("failed", (job, err) => {
  logger({
    jobId: job?.id,
    queue: "mfa-reminder-maintenance",
    attemptNumber: job?.attemptsMade,
  }).error({ err }, "job.failed");
  Sentry.captureException(err);
});
